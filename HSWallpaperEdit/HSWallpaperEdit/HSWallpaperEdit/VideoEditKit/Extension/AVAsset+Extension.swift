...
extension AVAsset {
    ...
    func makeStillImageTimeRange(percent:Float, inFrameCount:Int = 0) -> CMTimeRange {
        ...
        return CMTimeRangeMake(start: time, duration: CMTime.init(value: frameDuration, timescale: time.timescale))
    }
    ...
    func getAssetFrame(percent:Float) -> UIImage?
    {
        ...
        imageGenerator.requestedTimeToleranceAfter = CMTime.init(value: 1, timescale: 100)
        imageGenerator.requestedTimeToleranceBefore = CMTime.init(value: 1, timescale: 100)
