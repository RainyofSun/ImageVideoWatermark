...
class HSBaseEditView: UIView {
    ...
    // 定义焦点获取、失去、删除回调
    typealias BecomeFirstResponderClousre = ((HSBaseEditView) -> Void)
    typealias ResignFirstResponderClousre = (() -> Void)
    typealias DeleteElementClousre = (() -> Void)
    var becomeFirstResponderClousre: BecomeFirstResponderClousre?
    var resignFirstResponderClousre: ResignFirstResponderClousre?
    var deleteElementClousre: DeleteElementClousre?
    
    // 边框伸缩距离边界的最小值
    let contentLayoutConstant: CGFloat = 5
    // label标签左侧距离边框的距离
    let contentLabelLeftConstant: CGFloat = 10
    // 文本边框伸缩宽度的最小值
    let minTextContentLayoutWidthConstant: CGFloat = 80
    // 图片边框伸缩宽度的最小值
    let minImageContentLayoutWidthConstant: CGFloat = 80
    // 文字边框伸缩宽度的最大值
    let maxTextContentLayoutWidthConstant: CGFloat = K_SCREEN_WIDTH * 0.8
    // 图片边框伸缩宽度的最大值
    let maxImgContentLayoutWidthConstant: CGFloat = K_SCREEN_WIDTH * 0.7
    // 初始旋转角度
    var oldRadious: CGFloat = 0.0
    // 旋转角度
    var rotateRadious: CGFloat = 0.0
    // 旋转手势角度
    var rotateRadiousGesture: CGFloat = 0.0
    // 编辑类型 默认为 文字编辑
    var editType: HSEditType = .TEXT
    // 拖动放大的最初手势位置
    var panScaleBeginPoint: CGPoint = .zero
    // 四角操作位大小
    let badgeBounds: CGFloat = 20
    // 数据操作
    ...
    var attributeText: NSAttributedString? {
        get {
            return contentView.textLabel.attributedText
        }
    }
    
    public var topRightTagView: HSEditTagView {
        get {
            return topRightView
        }
    }
    ...
    override func becomeFirstResponder() -> Bool {
        ...
        if result {
            self.becomeActive()
            self.becomeFirstResponderClousre?(self)
        }
        return result
    }
    ...
    override func resignFirstResponder() -> Bool {
        ...
        self.resginActive()
        self.resignFirstResponderClousre?()
        return result
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        // 记录初始的Frame
        viewModel.viewCenter = self.center
        viewModel.viewBounds = self.bounds.size
        viewModel.viewLayerData.type = editType.rawValue
    }
    ...
    private func loadSubViews() {
        ...
        topLeftView.imgName = "iconClose"
        self.addSubview(topLeftView)
        
        topRightView.imgName = self.editType == .TEXT ? "iconEdit" : "iconEdit"
        self.addSubview(topRightView)
        
        // 暂不开放拖拽旋转功能
        bottomLeftView.imgName = "iconEdit"
        bottomLeftView.isHidden = true
        self.addSubview(bottomLeftView)
        
        bottomRightView.imgName = "iconDrag"
...
extension HSBaseEditView {
    ...
    @objc func panRotateHandle(sender: UIPanGestureRecognizer) {
        ...
        case.changed:
            ...
            oldRadious = radious
            viewModel.rotation = rotateRadious
    ...
    @objc func panScaleHandle(_ sender: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            let minConstant: CGFloat = self.editType == .IMAGE ? minImageContentLayoutWidthConstant : minTextContentLayoutWidthConstant
            if size.width * scale < minConstant {
                scale = 1.0
            }
            let maxConstant: CGFloat = self.editType == .IMAGE ? maxImgContentLayoutWidthConstant : maxTextContentLayoutWidthConstant
            if size.width * scale > maxConstant {
                return
            }
            if self.frame.height <= minTextContentLayoutWidthConstant * 0.6 && self.editType == .TEXT {
                return
            }
    ...
    @objc func pinchHandle(sender: UIPinchGestureRecognizer) {
        if sender.state == .began || sender.state == .changed {
            let minConstant: CGFloat = self.editType == .IMAGE ? minImageContentLayoutWidthConstant : minTextContentLayoutWidthConstant
            if self.frame.width * sender.scale > minConstant {
    ...
    @objc func rotationHandle(sender: UIRotationGestureRecognizer) {
        if sender.state == .began || sender.state == .changed {
            rotateRadiousGesture += sender.rotation
            viewModel.rotation = rotateRadiousGesture
