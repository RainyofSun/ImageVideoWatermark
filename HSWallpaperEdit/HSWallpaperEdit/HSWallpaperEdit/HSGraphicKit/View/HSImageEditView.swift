...
// MARK: 修改图片的亮度、等SliderView
class HSImageEditView: UIView {
    ...
    private lazy var brightnessIconView: UIImageView = {
        let view = UIImageView(image: UIImage.init(named: "icon_tool_Lightness_default"))
        return view
    }()
    
    private lazy var contrastIconView: UIImageView = {
        let view = UIImageView(image: UIImage.init(named: "icon_tool_contrastratio_default"))
        return view
    }()
