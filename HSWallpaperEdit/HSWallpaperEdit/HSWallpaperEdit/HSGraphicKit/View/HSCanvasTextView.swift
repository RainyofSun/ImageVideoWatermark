...
//    }
    ...
    public func getMaxContentSize() -> CGSize {
        ...
        contentRect.size.width = frame.width
        let textContentHeight = getTextContentHeight() + 20
        if textContentHeight > contentRect.height {
            // 文字获取的带有Padding需要返回 + 20
            contentRect.size.height = 20
        } else {
            // 加 Margin 10
            contentRect.size.height -= textContentHeight
        }
        return contentRect.size
    }
    ...
    // 超出视图边距后，依次向上偏移15,
    public func getUpOffsetBottomPoint() -> CGRect {
        var frame = (self.subviews.last?.frame.offsetBy(dx: 0, dy: -20))!
        let textContentHeight = getTextContentHeight() + 10
        if frame.origin.y < 50 {
            frame.origin.y = self.frame.height - textContentHeight
        }
        return frame
    }
